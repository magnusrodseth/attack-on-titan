...
- Game mode system (user request, 2026-07-09) — GameMode interface in `src/sim/modes.ts` (start/step/chooseUpgrade hooks over shared core systems); the original endless run extracted as "Wave Survival" with parity and determinism pinned by tests; registry-driven Game Mode panel in the menu; choice persists ('aot-odm-mode') and rides the URL as ?mode= (switching reloads into the new mode). Future modes (time trial, parkour rings) append to GAME_MODES without touching game.ts.
