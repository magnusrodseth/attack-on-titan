...
- Tethered ground momentum (user request, 2026-07-09) — grounded runs keep their speed while a hook is attached (rolling loss of 2 m/s² instead of the 8-12 skid); releasing while grounded restores the skid, and running past the anchor scoops the player back into the swing (emergent from the existing rope constraint plus winch, pinned by tests).
