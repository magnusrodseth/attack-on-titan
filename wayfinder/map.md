...
- Shifters every third wave, and the cadence is content now (user request, 2026-07-14, shipped):
  `BOSS_WAVE_INTERVAL` 5 → 3. At five, the Founding sat 45 waves out and effectively nobody ever
  met it; at three the whole ladder lands inside a run people actually finish (Beast 3, Cart 6,
  Founding 27). Two things had to be fixed for a one-line constant to be honestly one line.
  First, `BossSpec.wave` was nine hand-written wave numbers duplicating `slot × interval` — read
  only by tests and the playground, and a lie the instant the interval moved. Deleted; the debut
  wave is derived (`bossDebutWave`), and the tests that hardcoded waves 5/6/15 now speak in
  intervals, so the next cadence change touches one number. Second, and worse: `CONTENT_HASH` was
  built from **ids only**, so the interval could move without moving the hash — a stale Worker and
  a fresh client would pass the handshake and then disagree about which waves hold a Shifter, the
  client announcing a boss the server never spawned. The cadence is in the hash now, and the guard
  that was supposed to catch this asserted `CONTENT_HASH === CONTENT_HASH` (true of any constant,
  proof of nothing) — it now varies a fact and proves the hash actually moves. The rule the hash
